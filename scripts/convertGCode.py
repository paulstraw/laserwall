...
def run(input, output):
  ...
  for l in lines:
    ...
    # G1
      ...
      parts = l.split("Y")
      newposx = float(parts[0][4:])
      # newposy = parts[1][1:]
      newposy = parts[1]
      newposy = float(newposy.split(' ')[0])
      cmds.append((2,newposx,newposy))
      posx = newposx
      posy = newposy
    # G0 same as G1, but turns laser off before moving and on after
    elif l.startswith("G0 "):
      parts = l.split("Y")
      newposx = float(parts[0][4:])
      # newposy = parts[1][1:]
      newposy = parts[1]
...
if __name__ == "__main__":
  if len(sys.argv) < 3:
    print "Usage: convertGCode.py inputfile.nc outputfile.h"
  else:
